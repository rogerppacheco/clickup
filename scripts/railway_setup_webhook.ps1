# Cria/configura serviço Railway "site-clickup-webhook" (worker WhatsApp dedicado).
...
$WebService = if ($env:RAILWAY_WEB_SERVICE) { $env:RAILWAY_WEB_SERVICE } else { "site-clickup" }
$WebhookService = if ($env:RAILWAY_WEBHOOK_SERVICE) { $env:RAILWAY_WEBHOOK_SERVICE } else { "site-clickup-webhook" }
...
Write-Host "  Repo: rogerppacheco/site-clickup"
Write-Host ""
