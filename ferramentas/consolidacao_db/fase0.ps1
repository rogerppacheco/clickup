...
$RailwayCentral = @("-p", $ProjectCentral, "-s", "site-clickup", "-e", "production")
...
if ($pgDump) {
    ...
    $centralUrl = (railway variables --kv -p $ProjectCentral -s site-clickup -e production 2>$null | Where-Object { $_ -match '^DATABASE_URL=' }) -replace '^DATABASE_URL=', ''
