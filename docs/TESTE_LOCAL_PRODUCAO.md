...
```bash
cd c:\site-clickup   # ou clone do repositório
```
...
Crie ou edite o `.env` na raiz do projeto (`c:\site-clickup\.env`). Use como base o que está em produção e ajuste só o que for necessário:
...
2. Pelo **WhatsApp**, envie para o número da ClickUp/API:
   - **VENDER**
...
| Passo | O quê |
|-------|--------|
| 1 | Código em `c:\site-clickup`, venv, `pip install -r requirements.txt` |
